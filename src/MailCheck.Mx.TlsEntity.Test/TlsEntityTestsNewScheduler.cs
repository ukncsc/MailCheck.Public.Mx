...
{
    [TestFixture]
    public class TlsEntityNewSchedulerTests
    {
        private TlsEntityNewScheduler _tlsEntity;
        ...
        private ILogger<TlsEntityNewScheduler> _log;
        ...
        public void SetUp()
        {
            ...
            _log = A.Fake<ILogger<TlsEntityNewScheduler>>();
            ...
            _tlsEntity = new TlsEntityNewScheduler(_dao, _clock, _tlsEntityConfig, _dispatcher, _domainStatusPublisher, _entityChangedPublisher, _changeNotifierComposite, _log);
        }
        ...
        public async Task ShouldNotUpdateOrDispatchPollPendingWhenScheduledReminderArrivesBeforeTtl()
        {
            ...
            A.CallTo(() => _tlsEntityConfig.MinimumSchedulerInterval).Returns(2);
        ...
        [Test]
        public async Task ShouldDispatchCreateScheduledReminderWhenStateDoesntExist()
        {
            const string hostName = "testhostname";
            const string snsTopicArn = "snsTopicArn";

            A.CallTo(() => _dao.Get(hostName)).Returns((TlsEntityState) null);
            A.CallTo(() => _tlsEntityConfig.SnsTopicArn).Returns(snsTopicArn);

            await _tlsEntity.Handle(new TlsScheduledReminder(Guid.NewGuid().ToString(), hostName));

            A.CallTo(() => _dispatcher.Dispatch(A<CreateScheduledReminder>.That.Matches(_ => 
                _.ResourceId == hostName && _.Service == "Tls" && _.ScheduledTime == default), snsTopicArn)).MustHaveHappenedOnceExactly();
        }
